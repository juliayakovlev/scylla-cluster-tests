...
class SlaUtils:
    ...
    @staticmethod
    def role_to_scheduler_group(test_roles, scheduler_shares):
        """
         Map Service Level with its shares (collected from 'scylla_scheduler_shares', Prometheus) to the role
        """
        # TODO: handle case when few service levels are attached to the role
        # create dictionary in format: {'role250_362ee06e': [250, 'sl250'], 'role500_362ee06e': [500, 'sl500']}
        # where:
        #   - 250 - shares
        #   - 'sl250' - service level group
        for role, shares in test_roles.items():
            for scheduler_group, sg_shares in scheduler_shares.items():
                if shares[0] in sg_shares and scheduler_group not in test_roles[role]:
                    test_roles[role].append(scheduler_group)
                    break
            # Scheduler group with  is not found
            if len(test_roles[role]) != 2:
                ValueError(f"Service level group for role {role} with {shares[0]} shares was not found in Prometheus")
        # Example of output:
        # {'role20_0': [20, 'sl:sl20_0'], 'role500_0': [500, 'sl:sl500_0']}
        return test_roles
    ...
    def validate_scheduler_runtime(self, start_time, end_time, read_users, prometheus_stats, db_cluster,
        ...
        # TODO: ask Eliran do we really need validate it by every node?
        result = []
        for node_ip in db_cluster.get_node_private_ips():
            # # Temporary solution
            # scheduler_shares = prometheus_stats.get_scylla_scheduler_shares_per_sla(start_time, end_time, node_ip)
            # # Example: {'sl:default': {1000}, 'sl:sl500_596ca81a': {500}}
            # LOGGER.debug('SCHEDULERS SHARES FROM PROMETHEUS: {}'.format(scheduler_shares))
            # # this default scheduler that is not under test - ignore it
            # if 'sl:default' in scheduler_shares:
            #     scheduler_shares.pop('sl:default')
            #
            # test_users_to_sg = self.role_to_scheduler_group(test_roles=roles_full_info,
            #                                                 scheduler_shares=scheduler_shares)
            # # Example of test_users_to_sg:
            # # {'role250_596ca81a': [250, 'sl:sl250_596ca81a'],
            # # 'role500_596ca81a': [500, 'sl:sl500_596ca81a']}
            # LOGGER.debug('ROLE - SERVICE LEVEL - SCHEDULER: {}'.format(test_users_to_sg))
            #
            ...
            for role_sl_attribute in roles_full_info.values():
                if role_sl_attribute['sl_group'] in scheduler_runtime_per_sla[node_ip]:
                    role_sl_attribute['sl_group_runtime'] = sum(
                        ...
                        len(scheduler_runtime_per_sla[node_ip][
                            role_sl_attribute['sl_group']])
                else:
                    role_sl_attribute['sl_group_runtime'] = 0
            ...
            # TODO: next 5 lines will be used by sla_per_user_system_test.py. Will need be adapted
        ...
        if any(result):
            raise SchedulerRuntimeUnexpectedValue("\n".join(result))
    ...
    def validate_runtime_relatively_to_share(roles_full_info: dict, node_ip: str,
        ...
        # Runtime per role in runtime_per_role is always in the same order as it in test_users_to_sg (by role)
        # runtime_per_role example:
        #     {'role20_0': 239.64500346317064, 'role500_0': 279.78664617753225}
        ...
        if shares_ratio == runtimes_ratio:
            ...
            message = (f'(Node {node_ip}) - Role with higher shares got less resources unexpectedly.%s '
