    ...
    def _run_stress(self, node, loader_idx, cpu_idx, keyspace_idx):  # pylint: disable=too-many-locals
        ...
        with CassandraStressExporter(instance_name=node.ip_address,
                ...
                CassandraStressEvent(node=node, stress_cmd=self.stress_cmd,
                                     log_file_name=log_file_name) as cs_bench_event:
            publisher.event_id = cs_bench_event.event_id
            try:
                result = node.remoter.run(cmd=node_cmd, timeout=self.timeout, log_file=log_file_name)
            except Exception as exc:
                cs_bench_event.severity = Severity.CRITICAL if self.stop_test_on_failure else Severity.ERROR
                cs_bench_event.add_error(errors=[format_stress_cmd_error(exc)])

        return node, result, cs_bench_event
