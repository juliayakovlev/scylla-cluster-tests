...
class Nemesis:  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def __init__(self, tester_obj, termination_event, *args):  # pylint: disable=unused-argument
        ...
        # self.log.info("Wrapper id %s for cluster %s", str(id(disrupt_wrapper)), self.cluster.name)
    ...
    def run(self, interval=None, cycles_count: int = -1):
        ...
        while not self.termination_event.is_set():
            ...
            self.log.info("Cluster %s, nemesis thread id %s", self.cluster.name, id(self))
            try:
                self.disrupt()
                ...
                self.log.info("Cluster %s, nemesis thread id %s, target node %s", self.cluster.name, id(self),
                              self.target_node)
                self.unset_current_running_nemesis(self.target_node)
    ...
    # def wrapper_disrupt_method(self, method):  # pylint: disable=too-many-statements
    #     """
    ...
    #         # pylint: disable=too-many-locals
    #         # pylint: disable=too-many-branches
    #         args[0].log.debug('Wrapper with id %s started', str(id(self)))
    #         args[0].log.debug('Nemesis thread id %s', str(id(args[0])))
    #         args[0].log.debug('Current thread name: %s', threading.current_thread().name)
    ...
    #         args[0].log.debug('Wrapper with id %s before nemesis run', str(id(self)))
    #
    ...
    #         # TODO: Temporary print. Will be removed later
    #         data_validation_prints(args=args)
    #         args[0].log.debug('Wrapper with id %s finished', str(id(self)))
    #         return result
    #
    #     return wrapper
