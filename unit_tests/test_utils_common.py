...
from sdcm.utils.common import tag_ami, convert_metric_to_ms, LoadUtils
from sdcm.utils.common import download_dir_from_cloud
from data_dir.test_data_library import load_inventory
...
class TestLoadUtils(unittest.TestCase):
    temp_dir = None
    ...
    def test_load_column_1_data_inventory(self):
        test_data = get_load_test_data_inventory(column_number=1, big_sstable=False, load_and_stream=False)
        assert test_data == load_inventory.COLUMN_1_DATA, \
            f"Expected {load_inventory.COLUMN_1_DATA}, got {test_data}"

        test_data = get_load_test_data_inventory(column_number=1, big_sstable=True, load_and_stream=False)
    ...
    def test_load_multi_column_data_inventory(self):
        test_data = get_load_test_data_inventory(column_number=5, big_sstable=False, load_and_stream=False)
        assert test_data == load_inventory.MULTI_COLUMNS_DATA, \
            f"Expected {load_inventory.MULTI_COLUMNS_DATA}, got {test_data}"

        test_data = get_load_test_data_inventory(column_number=5, big_sstable=True, load_and_stream=False)
        ...
        test_data = get_load_test_data_inventory(column_number=6, big_sstable=False, load_and_stream=False)
        assert test_data == load_inventory.MULTI_COLUMNS_DATA, \
            f"Expected {load_inventory.MULTI_COLUMNS_DATA}, got {test_data}"

        test_data = get_load_test_data_inventory(column_number=6, big_sstable=True, load_and_stream=False)
        assert test_data == load_inventory.BIG_SSTABLE_MULTI_COLUMNS_DATA, \
            f"Expected {load_inventory.BIG_SSTABLE_MULTI_COLUMNS_DATA}, got {test_data}"

    def test_load_not_supported_data_inventory(self):
        test_data = get_load_test_data_inventory(column_number=2, big_sstable=False, load_and_stream=False)
        assert test_data is None, \
            f"Expected empty test data, got {test_data}"

    def test_load_load_and_stream_data_inventory(self):
        test_data = get_load_test_data_inventory(column_number=5, big_sstable=False, load_and_stream=True)
        assert test_data == load_inventory.MULTI_NODE_DATA, \
            f"Expected {load_inventory.MULTI_NODE_DATA}, got {test_data}"

    def test_distribute_test_files_to_cluster_nodes(self):
        ...
        for case in test_cases:
            test_data = get_load_test_data_inventory(5, big_sstable=False, load_and_stream=True)
            map_files_to_node = helpers.distribute_test_files_to_cluster_nodes(nodes=case["nodes"],
                                                                               test_data=test_data)
    ...
    def test_load_and_stream_status(self):
        helpers = LoadUtils()
        patterns = [helpers.LOAD_AND_STREAM_DONE_EXPR.format('keyspace1', 'standard1'),
                    helpers.LOAD_AND_STREAM_RUN_EXPR]
        system_log_follower = self.node.follow_system_log(start_from_beginning=True, patterns=patterns)
        helpers.validate_load_and_stream_status(self.node, system_log_follower)
