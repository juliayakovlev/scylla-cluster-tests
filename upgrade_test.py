...
def truncate_entries(func):
    ...
    def inner(self, *args, **kwargs):
        node = args[0]
        with self.db_cluster.cql_connection_patient(node, keyspace='truncate_ks', connect_timeout=300) as session:
        ...
        # re-new connection
        with self.db_cluster.cql_connection_patient(node, keyspace='truncate_ks', connect_timeout=240) as session:
            self.validate_truncated_entries_for_table(session=session, system_truncated=True)
...
class UpgradeTest(FillDatabaseData, loader_utils.LoaderUtilsMixin):
    ...
    def read_data_from_truncated_tables(self, session):
        ...
        for table_name in tables_name:
            ...
            try:
                count = self.rows_to_list(session.execute(truncate_query.format(table_name), timeout=300))
